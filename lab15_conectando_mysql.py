# Title     : Algoritmo Genético Completo -> INTEGRANDO COM BASE DE DADOS
# Objective : Neste laboratório vamos aprimorar nosso GA tornando-o mais próximo de uma aplicação real, onde poderemos ter milhões de registros
# Created by: accol
...
if __name__ =='__main__':
    lista_produtos = []
    #   Precisamos fazer a conexão com a base de dados
    conexao = pymysql.connect(host = 'localhost', user = 'angelo', passwd = '1234', db = 'produtos')
    cursor = conexao.cursor()   # Necessário para executar instruções SQL na base
    cursor.execute('select nome, espaco, valor, quantidade from produtos')
    for produto in cursor:
    #    print(produto[0])  # Só para teste da codificação
        for i in range(produto[3]): # Necessário para o controle da quantidade em estoque

            lista_produtos.append(Produto(produto[0], produto[1], produto[2]))
    
    # Como boa prática de programação devemos liberar a conexão e a variável cursor
    cursor.close()
    conexao.close()
    ...
    limite = 10  # Representa a capacidade máxima de transporte do caminhão 10 metros cubicos -> necessário para trabalhar com todo estoque (47 itens)
