# Title     : Seleção dos indivíduos
# Objective : Neste laboratório vamos trabalhar com a seleção de individuos para a formação de uma nova população.
# Created by: accol
# Created on: 11/06/2020


# Neste laboratório precisaremos selecionar os pais que darão início a uma nova população -> a partir dessa seleção deveremos aplicar os operadores genéticos (crossover e mutação) e evoluir enquanto espécie (solução para nosso problema)
# Seleção dos indivíduos:
#   Operadores genéticos são utilizados em indivíduos selecionados dentro da popupação
#   Indivíduos mais aptos (são aqueles que possuem uma função de avaliação => maior) serão selecionados mais frequentemente que is menos aptos -> as características dos melhores (mais aptos) deve predominar na nova  Geração
#   Deve simular o mecanismo de seleção natural que atua sobre as espécies biológicas: pais mais capazes     geram mais filhos e pais menos aptos também geram descendentes, porém em uma menor quantidade. Atenção     lembre-se que os menos aptos podem trazer em si potencial para mudanças ao cruzar com mais aptos
#   Vamos privilegiar os indivíduos com função de avaliação alta, sem desprezar completamente aqueles com    baixa avaliação
#   Atenção => se deixar apenas os melhores indivíduos, a população tenderá a ser composto de elementos cada vez mais semelhantes e faltará diversidade (requisito fundamental para evolução do nosso GA)

# Usaremos para a seleção dos indivíduos o MÉTODO DA ROLETA VICIADA
#   Neste método cada cromossomo (estamos usando aqui cromossomo para designar indivíduo -> dada a simplicidade do laboratório) recebe um pedaço proporcional à sua avaliação e a roleta é rodada
#   Etilismo => módulos de população que preservam os melhores, que garantem a estabilidade do melhor e não sua evolução
